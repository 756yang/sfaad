...
static int write_wav_header(audio_file *aufile)
{
    ...
    unsigned long data_size = (unsigned long)bytes * aufile->total_samples;
    unsigned long word32;

    *p++ = 'R'; *p++ = 'I'; *p++ = 'F'; *p++ = 'F';

    word32 = (data_size < MAXWAVESIZE) ?
        data_size + (44 - 8)  :  MAXWAVESIZE + (44 - 8);
...
static int write_wav_extensible_header(audio_file *aufile, long channelMask)
{
    ...
    *p++ = 'R'; *p++ = 'I'; *p++ = 'F'; *p++ = 'F';
    ...
    word32 = (data_size < MAXWAVESIZE) ?
        (unsigned long)data_size + (68 - 8)  :  (unsigned long)MAXWAVESIZE + (68 - 8);
    *p++ = (unsigned char)(word32 >>  0);
    *p++ = (unsigned char)(word32 >>  8);
    *p++ = (unsigned char)(word32 >> 16);
    *p++ = (unsigned char)(word32 >> 24);
                             ...
                             unsigned int samples)
{
    ...
    if (data) free(data);
#else
	int *sample_buffer_f = (int*)sample_buffer;
	aufile->total_samples += samples;
	if (aufile->channels == 6 && aufile->channelMask)
	{
	...
	for (int i = 0; i < samples; i++){
		if (sample_buffer_f[i] == 0x80000000) sample_buffer_f[i]=0;
	}
	int ret = fwrite(sample_buffer, samples, aufile->bits_per_sample / 8, aufile->sndfile);
#endif
    return ret;
}
